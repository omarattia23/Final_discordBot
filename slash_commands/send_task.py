...
class send_task(commands.Cog):
  ...
  @app_commands.command(name="send_task",
                        description="All tasks should be submitted here")
  async def send_task(self, interaction: discord.Interaction, task_number: str,
    ...
    try:
      ...
      if str(user_id) not in Mechanical_users:
      ...
      gfile = gdrive().CreateFile({
          ...
          file.filename,
          'parents': [{
              'id': folder_id[int(task_number) - 1]
          }],
          'content-length':
          file.size  # Set the content length explicitly
      })
